  ...
  function checkPassInput(id){
   ...
   function formatPhoneNumber(value) {
        ...
        function setDateLimits() {
                   ...
                   const fourteenYearsAgoStr = fourteenYearsAgo.toISOString().split('T')[0];

                   // 100년 전 날짜 계산
                   const oneHundredYearsAgo = new Date(today.getFullYear() - 100, today.getMonth(), today.getDate());
                   const oneHundredYearsAgoStr = oneHundredYearsAgo.toISOString().split('T')[0];

                   // input 요소 가져오기
                   const birthDateInput = document.getElementById('birthDate');

                   // min과 max 속성 설정
                   birthDateInput.min = oneHundredYearsAgoStr;
                   birthDateInput.max = fourteenYearsAgoStr;
               }
